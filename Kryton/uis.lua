...
--[Save Module]
local ConfigModule:table = {}
local Config = setmetatable({},{ __newindex = function(tables,key,value) rawset(tables,key,value) end})
local ConfigSetting = {FileName=game.Players.LocalPlayer.Name,AutoSave=true,FolderName='Folder Name Where The Scripts Go'}
function ConfigModule:CreateFile() :any
    ...
    if not isfile(FolderName..'/'..FileName..".json") then
        xpcall(function()
            writefile('Folder Name Where The Scripts Go/'..FileName..".json",tostring("{}"))
        end,function(x)
            return {false,x}
        end)
    else
...
function ConfigModule:LoadFile()
    local FileName = ConfigSetting.FileName or "Next"
    local FolderName = ConfigSetting.FolderName or "NextJs"
    if not isfolder(FolderName) then
...
function ConfigModule:Savefile()
    local FolderName = ConfigSetting.FolderName or "NextJs"
    local FileName = ConfigSetting.FileName or "Next"
    if not isfolder(FolderName) then
        ConfigModule:CreateFile()
		...
		function Tab.tab(options)
			...
			function Page.page(options)
				...
				game:GetService("RunService").Stepped:Connect(function()
					pcall(function()
						scrollingmainpage.CanvasSize = UDim2.new(0,0,0,scrollingmainpage_uilistlayout.AbsoluteContentSize.Y + 5)
				...
				function Item.Dropdown(options)
					...
					if not Mode then
						...
						function dropfunc.ClearWithName(ItemRemove:table)
							for k,c in next,ItemRemove do
								if TextHead.Name == c then
									for i,v in pairs(dropdownscrollingframe:GetChildren())do
										if v:IsA("TextButton") and TextHead.Name == c  then
											v:Destroy()
										end
									end
								end
						...
						function dropfunc.AddMod(Item:table)
							xpcall(function()
						...
						function dropfunc.Refresh(ItemRemove:table,additem:table)
							dropfunc.ClearWithName(ItemRemove);
							dropfunc.AddMod(additem);
						end
						function dropfunc.Update(Item:table)
							for i,v in next,Item do
								dropfunc.Add(v)
							end
						end
